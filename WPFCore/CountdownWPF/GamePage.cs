...
{
    public class GamePage : MultiPlayerWindow<CountdownViewModel, CountdownPlayerItem, CountdownSaveInfo>
    {
        ...
        public override Task HandleAsync(LoadEventModel message)
        {
            ...
            PlayerBoardWPF thisBoard = new PlayerBoardWPF();
        ...
        public override Task HandleAsync(UpdateEventModel message)
        {
            ...
            if (MainGame.ThisData!.MultiPlayer == false)
                ...
                ThisPlayer = MainGame.PlayerList!.GetSelf();
            PlayerBoardWPF? thisBoard = _playerStack.Children[0] as PlayerBoardWPF;
            thisBoard!.UpdateBoard(ThisPlayer);
            if (ThisPlayer.Id == 1)
                ThisPlayer = MainGame.PlayerList[2];
            else
                ThisPlayer = MainGame.PlayerList[1];
            thisBoard = _playerStack.Children[1] as PlayerBoardWPF;
            thisBoard!.UpdateBoard(ThisPlayer);
            EventAggregator thisE = OurContainer.Resolve<EventAggregator>();
            thisE.RepaintBoard(); //just in case.
            return Task.CompletedTask;
        }
        DiceListControlWPF<CountdownDice>? _diceControl; //i think.
