...
{
    public class BowlingCompleteScoresheetWPF : DisplayGameBoard
    {
        ...
        public void SavedScores() // rebindings.
        {
            ...
            foreach (var thisPlayer in _thisGame!.PlayerList!)
            {
                ...
                var thisLabel = (TextBlock)tempControl;
                thisLabel.Text = thisPlayer.NickName;
                for (y = 1; y <= 10; y++)
                {
                    var thisControl = FindControl(x, y);
                    var thisGraphics = (CompleteFrameWPF)thisControl;
        ...
        private CompleteFrameWPF GetControl(FrameInfoCP thisItem)
        {
            CompleteFrameWPF thisControl = new CompleteFrameWPF();
