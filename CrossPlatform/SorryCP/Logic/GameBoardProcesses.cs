...
namespace SorryCP.Logic
{
    ...
    public class GameBoardProcesses
    {
        ...
        public void StartTurn()
        {
            ...
            _gameContainer.SaveRoot.HighlightList = new CustomBasicList<int>();
            PreviousPiece = 0;
        ...
        {
            return _spaceList!.Values.Where(items => items.WhatBoard == EnumBoardStatus.OnBoard && items.Player > 0 && items.Player != _gameContainer.WhoTurn).ToCustomBasicList();
        }
        ...
        {
            return _spaceList!.Values.Where(items => items.WhatBoard != EnumBoardStatus.IsStart && items.Player == _gameContainer.WhoTurn).ToCustomBasicList();
        }
        ...
        private async Task SorryPlayerAsync(MoveInfo thisMove)
        {
            ...
            await AnimateTradeAsync(thisMove.SpaceFrom, thisMove.SpaceTo, _gameContainer.WhoTurn, _gameContainer.WhoTurn); //i think.
        ...
        private async Task EndMoveAsync()
        {
            ...
            {
                throw new BasicBlankException("Nobody is handling the startnewturn.  Rethink");
            }
        ...
        private async Task ResumeMoveAsync(int howMany, SpaceInfo thisSpace, MoveInfo thisMove)
        {
            ...
            _gameContainer.SingleInfo!.PieceList.RemoveSpecificItem(thisMove.SpaceFrom); //has to do this no matter what.  hopefully okay.
            if (thisSpace.Player == 0)
        ...
        public async Task GetValidMovesAsync()
        {
            ...
            if (MoveList.Count == 0)
            {
                if (_gameContainer.StartNewTurnAsync == null)
                {
                    throw new BasicBlankException("Nobody is handling the start new turn.  Rethink");
                }
