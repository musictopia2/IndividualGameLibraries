...
namespace AggravationCP.Graphics
{
    ...
    public class GameBoardGraphicsCP : BaseGameBoardCP<MarblePiecesCP<EnumColorChoice>>
    {
        ...
        public GameBoardGraphicsCP(AggravationGameContainer gameContainer) : base(gameContainer.Resolver)
        {
        ...
        protected override async Task ClickProcessAsync(SKPoint thisPoint)
        {
            if (_isTest == false)
            {
                if (_gameContainer.Command.IsExecuting || _gameContainer.SaveRoot.DiceNumber == 0)
                {
                    return;
                }
            }
            foreach (var thisSpace in _spaceList.Values)
            {
                ...
                if (MiscHelpers.DidClickRectangle(tempRect, thisPoint))
                {
                    ...
                    if (_isTest)
                    {
                        throw new BasicBlankException("If testing. then needs rethinking");
