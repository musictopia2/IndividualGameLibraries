...
namespace FluxxCP
{
    public class GoalCard : FluxxCardInformation
    {
        ...
        public int WhoWon(int HowManyExtra, FluxxMainGameClass mainGame)
        {
            if (_goal == null)
            ...
            if (_goal!.Item1 != EnumSpecialGoalSpecial.None)
            {
                ...
                if (_goal.Item1 == EnumSpecialGoalSpecial.Hand)
                    tempList = mainGame.PlayerList.Where(items => items.MainHandList.Count >= count).OrderByDescending(items => items.MainHandList.Count()).Take(2).ToCustomBasicList();
                else
                    tempList = mainGame.PlayerList.Where(items => items.KeeperList.Count >= count).OrderByDescending(items => items.KeeperList.Count).Take(2).ToCustomBasicList();
                if (tempList.Count == 0)
                    return 0;
                if (tempList.Count == 1)
            ...
            if (_goal.Item2.Count == 1)
            {
                ...
                thisPlayer = mainGame.PlayerList.Where(items => items.KeeperList.Count == 1 && items.KeeperList.Single().Deck == EnumKeeper.Love).SingleOrDefault();
            }
            else
            {
                if (_goal.Item2.Last().Item2 == true)
                {
                    if (mainGame.PlayerList.Any(tempPlayer => tempPlayer.KeeperList.Any(thisKeeper => thisKeeper.Deck == _goal.Item2.Last().Item1)))
                        return 0;
                    thisPlayer = mainGame.PlayerList.Where(tempPlayer => tempPlayer.KeeperList.Any(thisKeeper => thisKeeper.Deck == _goal.Item2.First().Item1)).SingleOrDefault();
                }
                else
                {
                    thisPlayer = mainGame.PlayerList.Where(tempPlayer => tempPlayer.KeeperList.Any
