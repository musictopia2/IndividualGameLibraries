...
namespace CribbageCP.Logic
{
    ...
    public class CribbageMainGameClass : CardGameClass<CribbageCard, CribbagePlayerItem, CribbageSaveInfo>, IMiscDataNM, IStartNewGame
    {
        ...
        private readonly CribbageGameContainer _gameContainer; //if we don't need it, take it out.
        private readonly RummyProcesses<EnumSuitList, EnumColorList, CribbageCard> _rummys;
        private readonly CustomBasicList<CribbageCombos> _comboList = new CustomBasicList<CribbageCombos>();
        ...
        public override async Task ContinueTurnAsync()
        {
            if (SaveRoot!.WhatStatus == EnumGameStatus.CardsForCrib)
            {
                ...
                if (BasicData!.MultiPlayer)
                    Check!.IsEnabled = false; //try this too.
                _model.NormalTurn = "None"; //because both players at the same time has to choose cards for crib.
        ...
        private void StartCardsCrib()
        {
            ClearItems();
            _model!.TotalScore = 0;
            _model.ScoreBoard1!.ResetScores(); //i think.
        }
        ...
        {
            _model!.PlayerHand1!.HandList.ReplaceRange(SingleInfo!.MainHandList); //somehow this was needed this time.
        }
        ...
        private async Task ResultsFromHandOrCribAsync()
        {
            ...
            else
            {
                ...
                SortCards(_model.CribFrame.HandList); //hopefully this works.
            }
            if (WhoTurn == 0)
        ...
        public CustomBasicList<CribbageCombos> ListComboPointsInPlay(int numberSoFar, out bool startOver, out int whoNext)
        {
            ...
            {
                if (_model.Pile1!.GetCardInfo().Value == EnumCardValueList.Jack)
                {
        ...
        public async Task PlayCardAsync(CribbageCard thisCard)
        {
            ...
            if (thisCard.Value < EnumCardValueList.Ten)
                _model.TotalCount += (int)thisCard.Value;
            else
