...
namespace ChineseCheckersCP
{
    ...
    public class GameBoardProcesses
    {
        ...
        private int WhoTurn
        {
            get
            {
                return _mainGame.SaveRoot!.PlayOrder.WhoTurn;
            }
            set
            {
                _mainGame.SaveRoot!.PlayOrder.WhoTurn = value;
            }
        }
        readonly ChineseCheckersMainGameClass _mainGame;
        readonly EventAggregator _thisE;
        public GameBoardProcesses(ChineseCheckersMainGameClass mainGame, EventAggregator thisE)
        {
            _mainGame = mainGame;
            _thisE = thisE;
        }
        private bool _turnContinued;
        private int CurrentSpace; // i think i need this (?)
        ...
        public async Task MakeMoveAsync(int index)
        {
            if (_graphicsBoard is null)
                _graphicsBoard = _mainGame.MainContainer.Resolve<GameBoardGraphicsCP>(); //i think.
            CurrentSpace = index;
            if (WhoTurn > _mainGame.PlayerList.Count())
                throw new BasicBlankException("Number cannot be higher than the total available players");
            _mainGame.SingleInfo!.PieceList.RemoveSpecificItem(_mainGame.SaveRoot!.PreviousSpace); // needs to remove.  will add later after animations
            _mainGame.Animates!.LocationFrom = _graphicsBoard.LocationOfSpace(_mainGame.SaveRoot.PreviousSpace);
            _mainGame.Animates.LocationTo = _graphicsBoard.LocationOfSpace(index);
            await _mainGame.Animates.DoAnimateAsync();
            _mainGame.SingleInfo.PieceList.Add(index); // you are now here instead
            _thisE.RepaintBoard();
        ...
        private async Task AdditionalProcesses(int index)
        {
            ...
            thisMove.SpaceTo = _mainGame.SaveRoot!.PreviousSpace;
            thisMove.SpaceFrom = _mainGame.SaveRoot.PreviousSpace;
            ...
            if (thisList.Count > 0)
            {
                _mainGame.SaveRoot.PreviousSpace = CurrentSpace;
                CurrentSpace = 0;
                _thisE.RepaintBoard();
                if (_mainGame.SingleInfo!.PlayerCategory == EnumPlayerCategory.Self)
                    _mainGame.SaveRoot.Instructions = "Choose to either end turn or make another move with the same piece";
        ...
        {
            _mainGame.SaveRoot!.PreviousSpace = index;
            _thisE.RepaintBoard();
            await _mainGame.ContinueTurnAsync(); // will continue turn from there.
        }
        private bool DidJump()
        {
            SpaceInfo thisSpace;
            SpaceInfo newSpace;
            thisSpace = _spaceList![_mainGame.SaveRoot!.PreviousSpace];
            if (thisSpace.LeftOnly > 0)
            {
                newSpace = _spaceList[thisSpace.LeftOnly];
                if (newSpace.SpaceNumber == CurrentSpace)
                    return false;
            }
        ...
        private CustomBasicList<MoveInfo> PossibleMovesWithPiece(SpaceInfo thisSpace)
        {
            ...
            if (thisSpace.LeftOnly > 0)
            {
                ...
                {
                    if (newSpace.WhatColor == EnumColorList.None || newSpace.DestinationColor == ourColor || newSpace.WhatColor == ourColor)
                    {
                ...
                if ((newSpace.Player > 0) & (newSpace.LeftOnly > 0) & (haveItem == false))
                {
                    ...
                    {
                        if (furtherSpace.WhatColor == EnumColorList.None || furtherSpace.DestinationColor == ourColor || furtherSpace.WhatColor == ourColor)
                        {
                            thisMove = new MoveInfo();
                            thisMove.SpaceFrom = thisSpace.SpaceNumber;
        ...
        private CustomBasicList<MoveInfo> MoveList()
        {
            ...
            var fins = _spaceList![_mainGame.SaveRoot.PreviousSpace];
        ...
        public void ClearBoard() // this will mean it will populate both lists.
        {
            ...
            foreach (var thisSpace in _spaceList!.Values)
            {
                if (thisSpace.WhatColor != EnumColorList.None)
                {
                    ChineseCheckersPlayerItem tempPlayer = _mainGame.PlayerList.SingleOrDefault(Items => Items.Color == thisSpace.WhatColor);
                    if (tempPlayer != null)
                    {
                        x += 1;
        ...
        public void LoadBoard() // hopefully this simple.
        {
            _mainGame.Animates!.LongestTravelTime = 80; // try 80.  50 could be too short.
            PopulateSpaceList();
        }
        ...
        private void PopulateSpaceList()
        {
            ...
            for (x = 1; x <= 121; x++)
            {
                ...
                thisSpace.WhatColor = EnumColorList.None;
                thisSpace.DestinationColor = EnumColorList.None;
                switch (x)
                {
                    case 1:
                        {
                            thisSpace.UpLeft = 2;
                            thisSpace.UpRight = 3;
                            thisSpace.WhatColor = EnumColorList.Green;
                            thisSpace.DestinationColor = EnumColorList.Blue;
                            break;
                        }
                    ...
                    case 11:
                        {
                            ...
                            thisSpace.WhatColor = EnumColorList.Purple;
                            thisSpace.DestinationColor = EnumColorList.Yellow;
                            break;
                        }
                    ...
                    case 20:
                        {
                            ...
                            thisSpace.WhatColor = EnumColorList.Gray;
                            thisSpace.DestinationColor = EnumColorList.Red;
                            break;
                        }
                    ...
                    case 66:
                        {
                            ...
                            thisSpace.DownLeft = x - 10;
                            thisSpace.DownRight = x - 9;
                            thisSpace.WhatColor = EnumColorList.Red;
                            thisSpace.DestinationColor = EnumColorList.Gray;
                            break;
                        }
                    ...
                    case 75:
                        {
                            ...
                            thisSpace.UpRight = x + 11;
                            thisSpace.DownLeft = x - 10;
                            thisSpace.WhatColor = EnumColorList.Yellow;
                            thisSpace.DestinationColor = EnumColorList.Purple;
                            break;
                        }
                    ...
                    case 112:
                        {
                            ...
                            thisSpace.WhatColor = EnumColorList.Blue;
                            thisSpace.DestinationColor = EnumColorList.Green;
                            break;
                        }
