...
{
    [SingletonGame]
    public class GameBoardGraphicsCP : BaseGameBoardCP<MarblePiecesCP<EnumColorList>>
    {
        private readonly Dictionary<int, SKRect> _spaceList = new Dictionary<int, SKRect>(); // i think this is fine for this one.
        private readonly ChineseCheckersMainGameClass _mainGame;
        private readonly ChineseCheckersViewModel _thisMod;
        public GameBoardGraphicsCP(IGamePackageResolver mainContainer) : base(mainContainer)
        {
            _mainGame = mainContainer.Resolve<ChineseCheckersMainGameClass>();
        ...
        protected override void ClickProcess(SKPoint thisPoint)
        {
            ...
            foreach (var thisSpace in _spaceList.Values)
            {
                ...
                if (MiscHelpers.DidClickRectangle(tempRect, thisPoint) == true)
                {
                    ...
                    if (_thisMod.SpaceCommand!.CanExecute(index) == true)
                    {
        ...
        {
            canvas.Clear();
            var thisColor = _mainGame.SingleInfo!.Color;
            MarblePiecesCP<EnumColorList> thisMarble = new MarblePiecesCP<EnumColorList>();
            ...
            foreach (var thisPlayer in _mainGame.PlayerList!) // would have to remove temporarily
            {
            ...
            if (_mainGame.Animates!.AnimationGoing == true)
            {
                thisMarble = GetGamePiece(_mainGame.SingleInfo.Color.ToColor(), _mainGame.Animates.CurrentLocation);
                thisMarble.DrawImage(canvas);
                return;
            }
        ...
        #endregion
        protected override MarblePiecesCP<EnumColorList> GetGamePiece(string color, SKPoint location)
        {
