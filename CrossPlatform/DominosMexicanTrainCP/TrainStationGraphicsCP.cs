...
namespace DominosMexicanTrainCP
{
    ...
    public class TrainStationGraphicsCP : BaseGameBoardCP
    {
        ...
        public TrainStationGraphicsCP(IGamePackageResolver mainContainer) : base(mainContainer)
        {
            _mainGame = mainContainer.Resolve<DominosMexicanTrainMainGameClass>();
        ...
        {
            return _mainGame.ThisMod.TrainStation1 != null;
        }
        ...
        {
            int index = _mainGame.ThisMod.TrainStation1!.TrainClicked(thisPoint);
            if (index == 0)
                return;
            _mainGame.ThisMod.TrainStationCommand!.Execute(index);
        }
        ...
        protected override void DrawBoard(SKCanvas canvas)
        {
            ...
            if (PrivateList.Count == 0)
                return;
            var tempDomino = _mainGame.ThisMod.TrainStation1!.CenterDomino;
            ...
            foreach (var thisTrain in _mainGame.ThisMod.TrainStation1.TrainList.Values)
            {
                x++;
                ...
                if (x == _mainGame.ThisMod.TrainStation1.Self)
                    canvas.DrawRect(tempRect, _redPaint);
                if (thisTrain.TrainUp || x == _mainGame.ThisMod.TrainStation1.Satisfy)
                {
                ...
                if (x == _mainGame.ThisMod.TrainStation1.TrainList.Count && thisTrain.TrainUp == false)
        ...
        public SKPoint DominoLocationNeeded(int index, int firstNumber, int secondNumber) // probably needs to recalculate that when it goes.
        {
            ...
            TrainInfo thisTrain;
            thisTrain = _mainGame.ThisMod.TrainStation1!.TrainList[index];
            bool doubles;
