...
namespace PaydayCP
{
    ...
    public class GameBoardGraphicsCP : BaseGameBoardCP<PawnPiecesCP<EnumColorChoice>>
    {
        ...
        private readonly GlobalFunctions _thisGlobals;
        private readonly PaydayViewModel _thisMod;
        public GameBoardGraphicsCP(IGamePackageResolver mainContainer) : base(mainContainer)
        {
            ...
            _mainGame = mainContainer.Resolve<PaydayMainGameClass>();
            _thisGlobals = mainContainer.Resolve<GlobalFunctions>();
            _thisMod = mainContainer.Resolve<PaydayViewModel>();
        }
        ...
        protected override void DrawBoard(SKCanvas canvas)
        {
            ...
            if (_mainGame.SaveRoot!.NumberHighlighted > 0)
            {
                if (_spaceList!.Keys.Contains(_mainGame.SaveRoot.NumberHighlighted.ToString()))
                {
                    var thisSpace = _spaceList[_mainGame.SaveRoot.NumberHighlighted.ToString()];
        ...
        protected override void ClickProcess(SKPoint thisPoint)
        {
            ...
            foreach (var thisSpace in _spaceList!.Values)
            {
                ...
                {
                    if (thisSpace.Number != _mainGame.SaveRoot!.NumberHighlighted)
                        return;
                    _thisMod.SpaceCommand.Execute(thisSpace.Number);
                    return;
                }
            }
        }
        ...
        private void DrawDates(SKCanvas canvas, SKRect bounds)
        {
            ...
            string thisText;
            thisText = _mainGame.SaveRoot!.LotteryAmount.ToCurrency(0);
        ...
        private void DrawLottery(SKCanvas canvas, SKRect bounds)
        {
            ...
            gp.AddPoly(pts);
            canvas.DrawPath(gp, _greenPaint);
            var textPaint = MiscHelpers.GetTextPaint(SKColors.White, bounds.Height / 4.2f, "Times New Roman");
            textPaint.FakeBoldText = true;
            float lefts;
            float tops;
            lefts = bounds.Left + (bounds.Width / 2);
