...
namespace CandylandCP
{
    ...
    public class CandylandBoardGraphicsCP : BaseGameBoardCP<CandylandPieceGraphicsCP>
    {
        ...
        public CandylandBoardGraphicsCP(IGamePackageResolver MainContainer) : base(MainContainer)
        {
            _thisGame = MainContainer.Resolve<CandylandMainGameClass>();
            _thisMod = MainContainer.Resolve<CandylandViewModel>();
            _gameBoard1 = MainContainer.Resolve<CandylandBoardProcesses>(); //looks like no overflow error this time.
            DrawBoardEarly = true;
        }
        ...
        private CustomBasicList<SKPath> arr_Paths = new CustomBasicList<SKPath>();
        private struct TempSpace
        {
            public int Number;
            public SKRect Bounds;
        }
        private CustomBasicList<TempSpace> arr_Spaces = new CustomBasicList<TempSpace>();
        private CustomBasicList<SKPoint> PositionList = new CustomBasicList<SKPoint>();
        ...
        protected override void CreateSpaces()
        {
            ...
            arr_Paths = new CustomBasicList<SKPath>();
            ...
            arr_Spaces.Add(finalSpace);
        }
        private void CreatePositions()
        {
            PositionList = new CustomBasicList<SKPoint>
            {
        ...
        private void CreatePaints()
        {
            ...
            _thisBit = ImageExtensions.GetSkBitmap(thisAssembly, "candylandboard.png");
            ExtraTop = -10;
            ExtraLeft = -4;
        }
        private CandylandPieceGraphicsCP GetPiece(CandylandPlayerItem thisPlayer) // this has to figure out location as well
        {
            var thisPos = PositionList[thisPlayer.SpaceNumber - 1];
        ...
        protected override void DrawBoard(SKCanvas canvas)
        {
            ...
            if (_thisGame.SingleInfo!.SpaceNumber > 0)
            {
                thisPiece = GetPiece(_thisGame.SingleInfo);
                thisPiece.DrawImage(canvas);
            }
        }
        ...
        {
            if (_thisMod.CastleClickCommand!.CanExecute(null!) == false)
                return;
            CandylandCardData thisCard;
            thisCard = _thisGame.SaveRoot!.CurrentCard!; //i think
            foreach (var thisItem in arr_Spaces)
            {
                bool rets;
                if (thisItem.Number < 127)
                {
                    var tempSpace = arr_Paths[thisItem.Number - 1];
                ...
                {
                    bool isValid;
                    isValid = _gameBoard1.IsValidMove(thisItem.Number, thisCard.WhichCard, thisCard.HowMany);
                    if (isValid == true)
                    {
                        if (thisItem.Number == 127)
                        {
                            _thisMod.CastleClickCommand.Execute(null!);
                            return;
                        }
                        else
                        {
                            _thisMod.SpaceClickCommand!.Execute(thisItem.Number);
                            return;
                        }
                    }
                }
