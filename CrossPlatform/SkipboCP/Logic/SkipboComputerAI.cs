...
namespace SkipboCP.Logic
{
    ...
    public class SkipboComputerAI
    {
        ...
        private DiscardPilesVM<SkipboCardInformation>? _tempDiscards; //looks like i need this one after all.
        ...
        private CustomBasicList<ComputerData> ValidMoveList()
        {
            ...
            thisCard = _model.StockPile!.GetCard(); // i think
            for (x = 1; x <= 4; x++)
            {
        ...
        private int BestCardForEmpty()
        {
            ...
            int wilds = _gameContainer.SingleInfo.MainHandList.Count(Items => Items.IsWild == true);
            if (wilds == _gameContainer.SingleInfo.MainHandList.Count)
                return _gameContainer.SingleInfo.MainHandList.First().Deck;
            DeckRegularDict<SkipboCardInformation> tempCol =
                _gameContainer.SingleInfo.MainHandList.Where(Items => Items.IsWild == false).ToRegularDeckDict();
