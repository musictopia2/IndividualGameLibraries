...
namespace SkipboCP
{
    ...
    public class SkipboComputerAI
    {
        ...
        private struct MoveInfo
        {
            ...
            // use use contents from collection to figure out if it 
            public int Value;
        ...
        private ComputerDiscardInfo FindBestWild()
        {
            ...
            foreach (var thisTemp in _tempDiscardList!)
            {
                ...
                {
                    var wildList = _mainGame.SingleInfo!.MainHandList.Where(Items => Items.IsWild == true).ToRegularDeckDict();
                    if (wildList.Count() == _mainGame.SingleInfo.MainHandList.Count)
                    {
                        // this means there are only wilds
                        thisCard = _mainGame.SingleInfo.MainHandList.First();
        ...
        private int MostOf(out int manys, IDeckDict<SkipboCardInformation> whatCol)
        {
            var temps = whatCol.GroupOrderDescending(Items => Items.Number);
            manys = temps.First().Count(); //i think
        ...
        private int BestCardForEmpty()
        {
            ...
            int wilds = _mainGame.SingleInfo.MainHandList.Count(Items => Items.IsWild == true);
            if (wilds == _mainGame.SingleInfo.MainHandList.Count)
                return _mainGame.SingleInfo.MainHandList.First().Deck;
            DeckRegularDict<SkipboCardInformation> tempCol =
                _mainGame.SingleInfo.MainHandList.Where(Items => Items.IsWild == false).ToRegularDeckDict();
            ...
            if (howMany == 1)
            {
                do
                {
                    ask1 = _rs.GetRandomNumber(12);
                    howMany = tempCol.Count(Items => Items.Number == ask1);
                    if (howMany > 0)
                    {
                        whatNum = ask1;
                        break;
            ...
            tempCol.KeepConditionalItems(Items => Items.Number == whatNum);
            return tempCol.First().Deck;
        }
        public SkipboComputerAI(SkipboMainGameClass mainGame, RandomGenerator rs)
        {
            _mainGame = mainGame;
            _rs = rs;
        }
        ...
        private SkipboCardInformation? FindDiscard(DiscardPilesVM<SkipboCardInformation> thisDiscard, int pile)
        {
        ...
        private MoveInfo ResultsOfMove(CustomBasicList<ComputerData> whatList, int whichOne)
        {
            ...
            do
            {
                ...
                if (newMove.WhichType == EnumCardType.IsNone)
                {
                    ...
                    {
                        CustomBasicList<ComputerData> tempCol = new CustomBasicList<ComputerData>();
