...
{
    public class LifeStoryHand : HandViewModel<LifeCardGameCardInformation>
    {
        ...
        private readonly LifeCardGameViewModel _thisMod;
        private readonly LifeCardGameMainGameClass _mainGame;
        public LifeStoryHand(IBasicGameVM thisMod, int player) : base(thisMod)
        {
            _thisGlobal = thisMod.MainContainer!.Resolve<GlobalClass>();
            _thisMod = (LifeCardGameViewModel)thisMod;
            GetPlayerIndex = player;
            AutoSelect = EnumAutoType.SelectOneOnly; //start with this.
            _mainGame = thisMod.MainContainer.Resolve<LifeCardGameMainGameClass>();
            _myID = _mainGame.PlayerList!.GetSelf().Id;
            HandList = new BasicGameFramework.BasicDrawables.Dictionary.DeckObservableDict<LifeCardGameCardInformation>(); //try here too.
        }
        private bool CanPositionCard(LifeCardGameCardInformation thisCard)
        {
            if (_thisGlobal.TradeList!.Count > 0)
                return _thisGlobal.TradeList.ObjectExist(thisCard.Deck);
            if (_thisGlobal.CardChosen == null == false)
            {
                return _thisGlobal.CardChosen!.Deck == thisCard.Deck;
            }
            if (Mode == EnumSelectMode.ChoosePlayer)
                return true;
            if (_thisMod.CurrentPile!.PileEmpty() == true)
                return true;
            var currentCard = _thisMod.CurrentPile.GetCardInfo();
        ...
        private async Task PlayerClickedAsync()
        {
            ...
            if (_mainGame.ThisData!.MultiPlayer)
                await _mainGame.ThisNet!.SendAllAsync("choseplayer", GetPlayerIndex);
            await _mainGame.ChosePlayerAsync(GetPlayerIndex);
        }
        protected override async Task PrivateBoardSingleClickedAsync()
        {
            if (AutoSelect == HandViewModel<LifeCardGameCardInformation>.EnumAutoType.SelectOneOnly)
                return;
            _didProcess = true;
            await PlayerClickedAsync();
        }
