...
namespace TileRummyCP
{
    public class TileSet : SetInfo<EnumColorType, EnumColorType, TileInfo, SavedSet>
    {
        ...
        public TileSet(IBasicGameVM thisMod) : base(thisMod)
        {
        ...
        public void CreateSet(IDeckDict<TileInfo> thisCol, EnumWhatSets whatType)
        {
            ...
            foreach (var tempTile in thisCol)
                _mainGame.SaveRoot!.TilesFromField.RemoveSpecificItem(tempTile.Deck);// if not there, ignore
            if (_setType == EnumWhatSets.Kinds)
            {
        ...
        public void AddTile(TileInfo thisTile, int position)
        {
            ...
            _mainGame.SaveRoot!.TilesFromField.RemoveSpecificItem(thisTile.Deck);
        ...
        public bool IsAcceptableSet()
        {
            ...
            if (_setType == EnumWhatSets.Runs)
                return _mainGame.Rummys!.IsNewRummy(newRummy, newRummy.Count, RummyProcesses<EnumColorType, EnumColorType, TileInfo>.EnumRummyType.Runs);
            return _mainGame.Rummys!.IsNewRummy(newRummy, newRummy.Count, RummyProcesses<EnumColorType, EnumColorType, TileInfo>.EnumRummyType.Sets);
        }
    }
}