...
namespace ClueBoardGameCP.Logic
{
    ...
    public class GameBoardProcesses
    {
        ...
        public PositionPieces Position
        {
            get
            {
                return _gameContainer.Position;
            }
            set
            {
                _gameContainer.Position = value;
            }
        }
        ...
        public void LoadSpacesInRoom()
        {
            ...
            for (x = 1; x <= 9; x++)
            {
                ...
                foreach (var ThisRect in ThisRoom.Space.ObjectList)
                {
                    try
                    {
                        Position.AddRectToArea(ThisRoom.Space, ThisRect); // try this way for now.
                    }
