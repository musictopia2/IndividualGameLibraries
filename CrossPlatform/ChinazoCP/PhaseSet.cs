...
namespace ChinazoCP
{
    public class PhaseSet : SetInfo<EnumSuitList, EnumColorList, ChinazoCard, SavedSet>
    {
        ...
        {
            CanExpandRuns = true;
            _mainGame = thisMod.MainContainer!.Resolve<ChinazoMainGameClass>();
        }
        ...
        public ChinazoCard CardNeeded(int position)
        {
            ...
            if (position == 1)
            {
                ...
                if (thisCard.Value == EnumCardValueList.Two)
                    return _mainGame.DeckList.First(items => items.SecondNumber == EnumCardValueList.LowAce && items.Suit == thisCard.Suit);
                return _mainGame.DeckList.First(items => (int)items.Value == thisCard.UsedAs - 1 && items.Suit == thisCard.Suit);
            }
            if (thisCard.Value == EnumCardValueList.HighAce || thisCard.Value == EnumCardValueList.LowAce)
                return new ChinazoCard();
            return _mainGame.DeckList.First(items => (int)items.SecondNumber == thisCard.UsedAs + 1 && items.Suit == thisCard.Suit);
        }
    }
}