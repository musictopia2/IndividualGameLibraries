        ...
        public void LoadBoard() // so at some point, this will run.
        {
            ...
            for (int_RowCount = 1; int_RowCount <= 10; int_RowCount++)
            {
                ...
                for (int_ColCount = 1; int_ColCount <= 10; int_ColCount++)
                {
                    ...
                    thisExp.Bounds = SKRect.Create(bounds.Location.X + ((bounds.Width * (int_ColCount - 1)) / 10), bounds.Location.Y + ((bounds.Height * (int_RowCount - 1)) / 10), ((bounds.Width * int_ColCount) / 10) - ((bounds.Width * (int_ColCount - 1)) / 10), ((bounds.Height * int_RowCount) / 10) - ((bounds.Height * (int_RowCount - 1)) / 10));
