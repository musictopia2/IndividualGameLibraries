...
namespace FroggiesCP.Logic
{
    ...
    public class FroggiesMainGameClass : IAggregatorContainer
    {
        ...
        public FroggiesMainGameClass(IEventAggregator aggregator,
            RandomGenerator rs
            )
        {
        ...
        public async Task NewGameAsync(FroggiesMainViewModel model)
        {
            CreateLilyPads(model);
            CopyToRedo();
            await NewLilyListAsync();
        ...
        private async Task LeapFrogAsync(LilyPadCP obj_Target, FroggiesMainViewModel model)
        {
            LilyPadCP obj_Leap;
            string str_Index = "";
            // Stop
            ...
            {
                await UIPlatform.ShowMessageAsync("No Moves Left");
                await NewGameAsync(model);
                model.MovesLeft = NumberOfMoves();
            }
        }
        public async Task ProcessLilyClickAsync(LilyPadCP obj_TempPad, FroggiesMainViewModel model)
        {
            LilyPadCP obj_TargetPad;
            LilyPadCP obj_LeapPad;
